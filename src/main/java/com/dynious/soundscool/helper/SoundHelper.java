...
public class SoundHelper
{
    public static double getSoundLength(File soundFile)
    {
        try
        {
            ...
            if (soundFile.getName().endsWith(".mp3")) return ((MP3AudioHeader) audioHeader).getPreciseTrackLength();
            else return ((GenericAudioHeader) audioHeader).getPreciseLength();
        }
        catch (Exception e)
        {
            e.printStackTrace();
    ...
    public static boolean isSoundInSoundsFolder(File soundFile)
    {
        ...
        if (path.endsWith("sounds") || path.endsWith("sounds" + File.separator)) return true;
    ...
    public static TagInfo getTagInfo(File soundFile)
    {
        ...
        if (title.length() != 0) tagInfo.setTitle(title);
        else tagInfo.setTitle(soundFile.getName());

        String artist = tag.getFirst(FieldKey.ARTIST).replaceAll("[^a-zA-Z0-9.-]", " ").trim();
        if (artist.length() != 0) tagInfo.setArtist(artist);
        else tagInfo.setArtist("Unknown Artist");
