...
public class SoundHandler
{
    ...
    public static ArrayList<Sound> guiRemoteList = new ArrayList<>();
    private static File soundsFolder;
    private static ConcurrentSkipListMap<SoundInfo, Sound> localSounds = new ConcurrentSkipListMap<>();
    private static TreeMap<SoundInfo, Sound> remoteSounds = new TreeMap<>();
    ...
    public static Sound getSound(SoundInfo soundInfo)
    {
        ...
        if (sound == null) return remoteSounds.get(soundInfo);
        return sound;
    }
    ...
    public static void findSounds()
    {
        ...
        new Thread(() -> addSoundsFromDir(soundsFolder)).start();
    }
    ...
    public static Sound setupSound(File file)
    {
        ...
        File category = new File("sounds" + File.separator + Reference.modid + File.separator + tagInfo.getArtist());
    ...
    {
        localSounds = new ConcurrentSkipListMap<>();
        remoteSounds = new TreeMap<>();
        findSounds();
    }
        ...
        for (File file : dir.listFiles())
        {
            if (file.isFile())
            {
                if (file.getName().endsWith(".ogg") || file.getName().endsWith(".wav") || file.getName().endsWith(".mp3"))
                {
                    ...
                    if (!localSounds.containsKey(sound.getSoundInfo())) localSounds.put(sound.getSoundInfo(), sound);
                }
            }
