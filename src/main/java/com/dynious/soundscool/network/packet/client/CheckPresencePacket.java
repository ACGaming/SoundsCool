    ...
    public static class Handler implements IMessageHandler<CheckPresencePacket, IMessage>
    {
        ...
        public IMessage onMessage(CheckPresencePacket message, MessageContext ctx)
        {
            ...
            if (player != null)
            {
                ...
                if (sound != null && message.request) NetworkHelper.serverSoundUpload(sound, player);
                else if (sound != null) return new SoundReceivedPacket(soundInfo);
                else return new SoundNotFoundPacket(soundInfo);
            }
            return null;
        }
    }
